...
try:
    ...
    if not df.empty:
        # CLEANUP: Remove NaN rows, Summary Rows, and ALL BLANK COLUMNS
        if len(df.columns) > 0 and df.columns[0] != 'Unnamed: 0':
            
            # SMART GHOST ROW DELETION: We now check the MAIN data column instead of the SR.NO column.
            identifier_col = None
            ...
            if identifier_col and identifier_col in df.columns:
                # If the main identifier column is blank, delete the row. This safely preserves DMC rows!
            ...
            # Destroy summary TOTAL rows to prevent double counting
            for col in df.columns[:5]:
                df = df[~df[col].astype(str).str.strip().str.upper().isin(['TOTAL', 'GRAND TOTAL'])]
            
            # Destroy all blank columns generated by Google Sheets
