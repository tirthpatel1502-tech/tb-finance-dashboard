...
try:
    ...
    if not df.empty:
        # AGGRESSIVE CLEANUP: Destroys Google Sheet ghost rows and empty dropdowns
        if len(df.columns) > 0 and df.columns[0] != 'Unnamed: 0':
            
            # 1. Regex Cleanup: If the first column (SR.NO) has hidden spaces or is empty, DELETE the row.
            df[df.columns[0]] = df[df.columns[0]].astype(str).replace(r'^\s*$', np.nan, regex=True).replace(['None', 'nan', 'NaN', '<NA>'], np.nan)
            df = df.dropna(subset=[df.columns[0]])
            
            # 2. Summary Row Cleanup
            for col in df.columns[:5]:
                df = df[~df[col].astype(str).str.strip().str.upper().isin(['TOTAL', 'GRAND TOTAL'])]
            
            # 3. Destroy all blank columns generated by Google Sheets
        ...
        # --- FILE TRACKER (NEW SUMMARY TAB) ---
            ...
            # EXTRA AGGRESSIVE FIX: Double check FILE NAME column specifically to delete ghost rows
